...
class MLPODECell_Autonomous(torch.nn.Module):
    ...
    def __init__(self, hidden_size, bias=True):
        ...
        print("Using wide MLP as ODE CELL")
        self.layers_stack = torch.nn.Sequential( torch.nn.Linear(hidden_size, 4*hidden_size, bias=True), torch.nn.ReLU(),
                                              torch.nn.Linear(4*hidden_size, 2*hidden_size, bias=True), torch.nn.ReLU(),
                                              torch.nn.Linear(2*hidden_size, hidden_size, bias=True), torch.nn.Tanh())
    ...
    def forward(self, t, h):
        ...
        Returns:
            Updated h
        """
        dh = self.layers_stack(h)
        return dh


class GRUBayesMLP(torch.nn.Module):
    """
    Classical MLP to be used in place of GRU-Bayes for ablation study.
    """
    def __init__(self, input_dim, output_dim):
        """
        input_dim is the dimension of the processed input.
        output_dim is the dimension of the hidden process.
        """
        super().__init__()
        self.mlp = torch.nn.Sequential(

                torch.nn.Linear(input_dim + output_dim, output_dim),
                torch.nn.Tanh(),
                torch.nn.Linear(output_dim, output_dim)
                )
    def forward(self, input_prep, previous_hidden):
        x = torch.cat((input_prep, previous_hidden),1)
        y = self.mlp(x)
        return y
...
class GRUObservationCellLogvar(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, prep_hidden, bias=True, MLP = False):
        super().__init__()
        ...
        if MLP:
            print("MLP Go")
            self.gru_d = GRUBayesMLP(prep_hidden * input_size, hidden_size) 
    ...
    def forward(self, h, p, X_obs, M_obs, i_obs):
        ...
        mse          = (torch.pow((X_obs - mean),2)*M_obs).sum()
...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1, classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, full_gru_ode=False, solver="euler", impute = True, mlp_ode = False, **options):
        """
        ...
        if "GRU_Bayes_MLP" in options:
            GRU_Bayes_MLP = options["GRU_Bayes_MLP"]
        else:
            GRU_Bayes_MLP = False
        ...
        if full_gru_ode:
            ...
            print("Using the MLP ODE")
            self.gru_c = MLPODECell_Autonomous(hidden_size)
        ...
        if logvar:
            self.gru_obs = GRUObservationCellLogvar(input_size, hidden_size, prep_hidden, bias=bias, MLP = GRU_Bayes_MLP)
        else:
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
                return_path=False, smoother = False, class_criterion = None, labels=None, h_init = None, t_init = None, return_mse = False):
        """
        Args:
        ...
        if return_path:
            ...
            path_p = [p]
            path_h = [h]
            X_before = torch.zeros_like(X) # Estimates of the model before GRU-Bayes
        ...
        for i, obs_time in enumerate(times):
            ...
            h, losses, mse_i = self.gru_obs(h, p, X_obs, M_obs, i_obs)
        ...
        if return_path:
            if return_mse: #Returns the sum of squared errors before GRU-Bayes.
                return h, loss, mse, class_pred, np.array(path_t), torch.stack(path_p), torch.stack(path_h), eval_times_total, eval_vals_total
            else:
                if smoother:
...
class Discretized_GRU(torch.nn.Module):
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
                return_path=False,h_init = None, smoother = False, class_criterion = None, labels=None):
        """
        Args:
            times      np vector of observation times
            time_ptr   start indices of data for a given time
        ...
        if return_path:
            if smoother:
                ...
                return h, loss, mse, class_pred, np.array(path_t), torch.stack(path_p), torch.stack(path_h)
        else:
            if smoother:
                return h, loss, class_pred, class_loss_vec
            else:
