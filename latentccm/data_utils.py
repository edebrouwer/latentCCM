...
def add_noise_to_df(df_in, noise_std):
    df = df_in.copy()
    for val_c in [c for c in df.columns if "Value" in c]:
        col_val_num = val_c.split("_")[1]
        df.loc[df[f"Mask_{col_val_num}"]==1,val_c] +=  noise_std*np.random.randn(df.loc[df[f"Mask_{col_val_num}"]==1].shape[0])
    return df
...
class ODE_Dataset(Dataset):
    ...
    """
    def __init__(self, csv_file=None, cov_file=None, label_file=None, panda_df=None, cov_df=None, label_df=None, root_dir="./", t_mult=1.0, idx=None, jitter_time=0, validation = False, val_options = None, rescale = 1, noise_std = 0):
        """
        Args:
            ...
            rescale : multiply the times by this constant.
            noise_std : std of a gaussian noise to apply on all observed data (only once - not minibatching)
        ...
        if noise_std >0:
            print(f"Noise added to data with std : {noise_std}")
            self.df = add_noise_to_df(self.df,noise_std)
        ...
        #rescaling if asked
        self.df["Time"] = self.df.Time*rescale
        ...
        if self.validation:
            ...
            if val_options.get("T_val_from"): #Validation samples only after some time.
                ...
                assert val_options.get("T_val_end") is None, "Cannot combine both options at the moment !"
            elif val_options.get("T_val_end"):
                df_afterIdx = self.df.loc[((self.df["Time"]>val_options["T_val"])&(self.df["Time"]<=val_options["T_val_end"])),"ID"].unique()
            else:
        ...
        self.df["Delta_t"] = self.df.Time.diff()
        self.df.loc[self.df.Time==0,"Delta_t"] = 0
        ...
        if self.validation:
            ...
            if val_options.get("T_val_from"): #Validation samples only after some time.
                ...
                self.df_after = self.df.loc[((self.df["Time"]>val_options["T_val"]) & (self.df["Time"]<=val_options["T_val_end"]))].sort_values("Time").copy()
            else:
            ...
            self.df_after.sort_values(["ID","Time"], inplace=True)
        else:
            self.df_after = None
        ...
        self.df.sort_values(["ID","Time"], inplace=True)

        print(self.df.columns)
        #df2 = self.df.reset_index().copy()
        #df2["Delta_t"] = df2.Time.diff()
        #df2.loc[df2.Time==0,"Delta_t"] = 0

        #self.df = df2.copy().set_index("ID")
...
def downsample_df(df,subsample_ratio):
    num_samples = df.shape[0] // subsample_ratio
    sample_times = np.random.choice(df.shape[0],num_samples, replace = False)
    return df.iloc[sample_times].copy()

def custom_subsample_collate_fn(batch, subsample_ratio):
    idx2batch = pd.Series(np.arange(len(batch)), index = [b["idx"] for b in batch])

    pat_idx   = [b["idx"] for b in batch]
    df        = pd.concat([downsample_df(b["path"],subsample_ratio) for b in batch],axis=0)
    df.sort_values(by=["Time"], inplace=True)
    ...
    if batch[0]['val_samples'] is not None:
        df_after = pd.concat(downsample_df(b["val_samples"],subsample_ratio) for b in batch)
        df_after.sort_values(by=["ID","Time"], inplace=True)
        value_cols_val = [c.startswith("Value") for c in df_after.columns]
        mask_cols_val  = [c.startswith("Mask") for c in df_after.columns]
        X_val = torch.tensor(df_after.iloc[:,value_cols_val].values)
...
def discrete_collate_fn(batch, delta_t = True, validation = False):
    """
    Discrete (RNN) version of the collate function. 
    If delta_t is True, returns the difference of time between observations in the observation vector in a packed sequence.
    ...
    value_cols_names = list(batch[0]["path"].columns[value_cols])
    mask_cols_names = list(batch[0]["path"].columns[mask_cols])

    ids = [b["idx"] for b in batch]
    
    if delta_t:
        paths = [torch.tensor(b["path"][["Delta_t"]+value_cols_names+mask_cols_names].values) for b in batch]
        if validation:
            val_paths = [torch.tensor(b["val_samples"][["Delta_t"]+value_cols_names + mask_cols_names].values) for b in batch]
    else:
        paths = [torch.tensor(b["path"][value_cols_names+mask_cols_names].values) for b in batch]
        if validation:
            val_paths = [torch.tensor(b["val_samples"][value_cols_names + mask_cols_names].values) for b in batch]


    cov= [b["cov"] for b in batch]
    y = [b["y"] for b in batch]

    
    sorted_paths, sorted_cov, sorted_y, sorted_ids = zip(*sorted(list(zip(paths,cov,y, ids)),key = lambda x: len(x[0]), reverse = True))


    df_cov    = torch.Tensor(list(sorted_cov))

    labels  = torch.tensor(list(sorted_y))

    mask_path = None
    val_mask_path = None

    if delta_t:
        packed_path = torch.nn.utils.rnn.pack_sequence(sorted_paths)
        if validation:
            packed_path = torch.nn.utils.rnn.pad_sequence(paths)
            mask_path = packed_path.max(dim=2).values!=0
            packed_val_samples = torch.nn.utils.rnn.pad_sequence(val_paths)
            val_mask_path = packed_val_samples.max(dim=2).values!=0
            
    else:
        packed_path = torch.stack(paths).transpose(0,1)
        if validation:
            packed_val_samples = torch.stack(val_paths).transpose(0,1)
    ...
    res["indices"]   = sorted_ids
    if validation:
        res["val_X"]    = packed_val_samples
        res["val_mask_path"] = val_mask_path
    return res
...
def compute_label(csv_file, idx, val_options):
    df = pd.read_csv('./'+csv_file)
    df_sub = df.loc[df.ID.isin(idx)].copy()
    df_sub = df_sub.loc[df_sub.Time>val_options['T_val']].copy()
    label = (df_sub.groupby("ID").Mask_2.max()==1).astype(int).reset_index()
    label.rename(columns = {"Mask_2":"label"}, inplace = True)
    label.to_csv("./subset_val_label.csv",index = False)
